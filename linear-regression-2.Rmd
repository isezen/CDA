...
get_metrics <- function(fit) {
  ...
  res.se <- round(res_std_err(fit), 2)
  return(c(RMSE = rmse, MAE = mae, Res.SE = res.se))
}
```
...
p <- data.frame(n = 1:nrow(aq), Original = aq$Ozone, fit1 = predict(fit1))
...
```{r, echo = FALSE}
plot(spca$importance[2,] * 100, type = "b", bty = "n", ylab = "proportion of variance (%)", xlab = "PC")
```
...
dat.pca <- cbind(aq$Ozone, pca$x)
pairs(dat.pca, panel = panel, upper.panel = upper.panel, diag.panel = diag.panel,
      log = 1, cex = 0.7, pch = 20, gap = 0)
```
...
```{r, echo = FALSE, fig.width = 7, fig.height = 3.5, out.width='90%'}
dat.res <- sapply(list(fit1, fit2, fit3, fit4), get_res)
colnames(dat.res) <- paste0("fit", 1:ncol(dat.res))
dat.res <- as.data.frame(cbind(n = 1:nrow(dat.res), dat.res))
dat.res <- reshape2::melt(dat.res, id.vars = 1, variable.name = "Models", 
                          value.name = "Residuals")

# ggplot(dat.res, aes(n, Residuals, col = Models)) +
#   geom_point(shape = 1) +
#   geom_hline(yintercept = 0, linetype = "dashed")

ggplot(dat.res, aes(Residuals, col = Models)) +
  stat_ecdf(geom = "step") +
